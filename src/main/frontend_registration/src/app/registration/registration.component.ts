...
export class RegistrationComponent implements OnInit {
  ...
  onFormSubmit(event) {
    ...
    const encrypted: string = CryptoJS.AES.encrypt(this.user.password, key, {iv: iv}).toString();
    let tUser: User;
    tUser = Object.assign({}, this.user);
    tUser.password = encrypted;

    // Зашифровка поля подтверждения пароля
    const encrypted2: string = CryptoJS.AES.encrypt(this.confirmPassword, key, {iv: iv}).toString();
